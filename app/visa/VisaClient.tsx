...
export default function VisaClient() {
  ...
  const checklist = useMemo(() => {
    ...
    if (key in baseChecklist) list.push(...baseChecklist[key]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid md:grid-cols-3 gap-3">
        <input className="border rounded-xl px-4 py-3" placeholder="Destination country (India, Nepal, UAE, Russia)" value={country} onChange={(e) => setCountry(e.target.value)} />
        <input className="border rounded-xl px-4 py-3" placeholder="Your nationality" value={nationality} onChange={(e) => setNationality(e.target.value)} />
        <input className="border rounded-xl px-4 py-3" placeholder="Procedure (e.g., Cardiac surgery)" value={procedure} onChange={(e) => setProcedure(e.target.value)} />
      </div>
      <ul className="list-disc ml-5">{checklist.map((item) => (<li key={item}>{item}</li>))}</ul>
      <p className="text-sm text-slate-600">Need help? Our visa concierge can assist with forms, appointments, and translations.</p>
    </div>
  );
}
